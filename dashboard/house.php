  ...
  include '../session.php';
    ...
    if (isset($_POST['approved'])){
      ...
      include '../database/db.php';
        ...
        $query              = "UPDATE house SET status=1 WHERE id=:id";
        $stmt               = $db->prepare($query);
        $stmt               -> bindValue(':id',$id, PDO::PARAM_STR);
        $result             = $stmt->execute();

        if ($result) {
        ?><script type="text/javascript">alert("House add is approved.");</script><?php
        header("Refresh:0");
      }else{
    ...
    if (isset($_POST['disapproved'])){
      ...
      include '../database/db.php';
        ...
        $query              = "UPDATE house SET status=4 WHERE id=:id";
        $stmt               = $db->prepare($query);
        $stmt               -> bindValue(':id',$id, PDO::PARAM_STR);
        $result             = $stmt->execute();

        if ($result) {
        ?><script type="text/javascript">alert("House add is disapproved.");</script><?php
        header("Refresh:0");
      }else{
        ?><script type="text/javascript">alert("Oops! something went wrong.");</script><?php
      }
                    ...
                    <td>
                      ...
                      <form action="house.php" method="POST">
                        <div class="card-image">
                          ...
                          <button class="btn btn-success btn-sm" type="submit" name="approved">Approved</button>
                          <button class="btn btn-danger btn-sm" type="submit" name="disapproved">Rejected</button>
