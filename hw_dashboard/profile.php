<?php 
  include '../session.php';
  	...
  	if (!isset($_SESSION['house_owner_id'])) {
	    ...
	    $query    = "SELECT * FROM house_owner WHERE id = {$_SESSION['house_owner_id']}";
    ...
    $name = $_POST['name'];
		...
		if($_FILES['photo']['size'] != 0 && $_FILES['photo']['error'] == 0){
	        ...
	        $target_file = hash_hmac('sha256', $target_file, 'house');
	        ...
	        if($check !== false) {
	            ...
	            if ($uploadOk == 0) {
                  ...
                  $_SESSION['house_owner_photo'] = $newfilename;

	            }
	        }

		} else{
			$queryProfile  = "SELECT * FROM house_owner WHERE id = {$_SESSION['house_owner_id']}";
      ...
      $query    = "UPDATE house_owner SET name = :name, phone =:phone, photo = :photo WHERE id   = :id";
      ...
      $stmt     -> bindValue(':id',$_SESSION['house_owner_id'],PDO::PARAM_STR);
	...
	if (isset($_POST['update-password'])) {
		...
		if($password != $current_pass){
			...
			//update into database
	        $query= "UPDATE house_owner SET password = :password WHERE id = :id";
