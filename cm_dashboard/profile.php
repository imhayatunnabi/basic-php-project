<?php 
  include '../session.php';
  	...
  	if (!isset($_SESSION['customer_id'])) {
	    ...
	    $query    = "SELECT * FROM customer WHERE id = {$_SESSION['customer_id']}";
    ...
    $name = $_POST['name'];
		...
		if($_FILES['photo']['size'] != 0 && $_FILES['photo']['error'] == 0){
	        ...
	        if($check !== false) {
	            ...
	            if ($uploadOk == 0) {
                  ...
                  $_SESSION['customer_photo'] = $newfilename;

	            }
	        }

		} else{
			$queryProfile  = "SELECT * FROM customer WHERE id = {$_SESSION['customer_id']}";
      ...
      $query    = "UPDATE customer SET name = :name, phone =:phone, photo = :photo WHERE id   = :id";
      ...
      $stmt     -> bindValue(':id',$_SESSION['customer_id'],PDO::PARAM_STR);
